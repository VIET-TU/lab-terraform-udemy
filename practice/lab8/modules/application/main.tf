...
resource "aws_lb" "alb1" {
  ...
  drop_invalid_header_fields = true // option chi tiet trong phan command.txt
  enable_deletion_protection = false // option // (true) This will prevent Terraform from AWS CLI // note
...
resource "aws_cloudwatch_metric_alarm" "cpu_alarm" {
  ...
  alarm_description   = "This metric monitors the average CPU usage and triggers scale-up when above 50%."
  dimensions = {
...
resource "aws_cloudwatch_metric_alarm" "cpu_alarm_low" {
  ...
  alarm_description   = "This metric monitors the average CPU usage and triggers scale-in when below 50%."
  dimensions = {
    AutoScalingGroupName = aws_autoscaling_group.asg.name
  }
