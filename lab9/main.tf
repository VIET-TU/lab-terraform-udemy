terraform {
  ...
  backend "s3" { # save states on s3
    bucket         = "udemy-terraform-state-singapore-linh"
    key            = "udemy-terraform" // name folder
    region         = "ap-southeast-1"
    dynamodb_table = "udemy-terraform-state" // dat lock -> tranh 2 người tươc tác cùng một lúc // phải tạo dynamodb table
  }
}
provider "aws" {
  region = var.region
